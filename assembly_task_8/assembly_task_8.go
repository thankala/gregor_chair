...
func main() {
	...
	robot3Controller :=
		controllers.NewRobotController(
			redisStorer,
			httpClient,
	...
	e.Spawn(services.NewAssemblyTaskActor[AssemblyTaskServices.AssemblyTask8Actor](
		AssemblyTaskServices.NewAssemblyTask8Actor(*robot3Controller), server),
...
func getKafkaOptions() []configuration.KafkaOptionFunc {
	...
	kafkaOptions = append(kafkaOptions, configuration.WithKafkaTopic(enums.AssemblyTask8.String()))
	kafkaOptions = append(kafkaOptions, configuration.WithKafkaGroupId(enums.AssemblyTask8.String()))
